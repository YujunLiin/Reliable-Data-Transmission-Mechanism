...
{   //设置本地缓存和变量
	...
	{   //第三次握手
		...
		*send_buffer = { 0 };
		...
		cache_data cache[100];//用于缓存发送了但是尚未被ack的数据内容和数据大小
			...
			else
			{
				...
				if ((recv_ack >= base) && (recv_ack < nextseqnum) && !(Corrupt(recv_buffer, ret)))
				{
					for (int i = base; i <= recv_ack; i++)
						cout << "成功接收序号为" << i << "大小为" << cache[(i - base + cache_base) % send_window_size].package_length << "的数据块" << endl;
					//判断是不是结束文件发送,若是的话状态应该是等待最后一次ack并且nextseqnum是最后的数据块序号+1，并且结束发送
					if (state == FILE_LAST_ACK && recv_ack == nextseqnum - 1)
					{
						cout << "成功发送总长为" << filelength << "的" << command[1] << "文件" << endl;
						end = clock();
					...
					if (nextseqnum >= base + send_window_size)
						continue;
					//窗口有剩余且数据没有发完，继续发送完剩下窗口
					while (nextseqnum < base + send_window_size)
					{
